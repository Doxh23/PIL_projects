...
function rednao_smart_forms_save()
{
    ...
    if(!wp_verify_nonce($nonce,'save_nonce'))
    {
...
function rednao_smart_forms_get_form_options()
{
    ...
    global $current_user;
    echo json_encode(array(
        'formOptions'=>strval($result[0]->form_options),
        'elementOptions'=>strval($result[0]->element_options),
...
function rednao_smart_forms_export_entries(){
    ...
    if(!wp_verify_nonce($data->Nonce,'smart_forms_export'))
    {
        echo "Invalid request";
        die();
    }

...
function rednao_smart_forms_load_form($title,$form_id,$returnComponent)
{
    ...
    if($returnComponent==false)
    {
        ...
        $loadedForm="";
        if($form_id!=-1)
            $loadedForm="window.smartFormsItemsToLoad.push({ 'form_id':".$options['form_id'].",  'elements':".str_replace("\xe2\x80\xa8", '\\u2028',$options['elements']).",'client_form_options':".str_replace("\xe2\x80\xa8", '\\u2028',$options['client_form_options']).",'container':'formContainer$random'});";
        $script="
